...
class Views:
    ...
    def show_profile(self, account: object) -> object:
        embed_content = self.load_embed("profile")
        embed_content["thumbnail"] = account.avatar_url
        embed_content["title"] = embed_content["title"].format(account.name)
        embed_content["content"]["`character name:`"] = account.name
        
        return self.create_embed(embed_content)